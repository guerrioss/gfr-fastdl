...
main()
{
	...
	maps\zombie_free_fx::main();
	maps\zombie_free_amb::main();
	...
	level.zombie_anim_override = maps\zombie_free::anim_override_func;
	...
	level.exit_level_func = ::zombie_free_exit_level;
	...
	maps\_compass::setupMiniMap("menu_map_zombie_free"); 
	level.ignore_spawner_func = ::zombie_free_ignore_spawner;
	level.zone_manager_init_func = ::zombie_free_zone_init;
...
zombie_free_ignore_spawner( spawner )
{
	// no power, no quads
	if ( !flag("power_on") )
	{
...
zombie_free_exit_level()
{
	zombies = GetAiArray( "axis" );
	for ( i = 0; i < zombies.size; i++ )
	{
		zombies[i] thread zombie_free_find_exit_point();
	}
}
zombie_free_find_exit_point()
{
	self endon( "death" );
	player = getplayers()[0];
	dist_zombie = 0;
